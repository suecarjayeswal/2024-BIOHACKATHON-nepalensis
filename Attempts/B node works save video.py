...
def update_graph(frame_data):
    for detection in frame_data['detections']:
        ...
        # If node is already present, update it; if not, add it
        if node_id in G:
            G.nodes[node_id]['emotion'] = emotion
            G.nodes[node_id]['percentage'] = percentage
            G.nodes[node_id]['bbox'] = detection['bbox']  # Update bounding box
        else:
            # Add new node
...
def update_frame(frame_number):
    ...
    # Draw nodes only for those that are detected in the current frame
    for node in G.nodes:
...
# Set up the FFMpeg writer for saving the video
writer = FFMpegWriter(fps=10, metadata=dict(artist="Matplotlib", title="Graph Video"), bitrate=1800)
...
output_video_path = 'output_animation.mp4'  # Specify the output path for the video
ani.save(output_video_path, writer=writer)

# Show the animation (optional, as you're saving it to a file)
plt.show()
