        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if not opt.no_save or not opt.hide_img:
                        
                        # Draw bbox with emotion label
                        plot_one_box(xyxy, im0s, label=label, color=colors[int(cls) % len(colors)], line_thickness=opt.line_thickness)


        # Show the frame with detections
        if not opt.hide_img:
            cv2.imshow("Emotion Detection", im0s)
            if cv2.waitKey(1) & 0xFF == ord('q'):  # Press 'q' to exit
                break
        
        # Save the frame 
        if not opt.no_save:
            if vid_path != opt.output_path:  # new video
                vid_path = opt.output_path
                ...
                vid_writer = cv2.VideoWriter(opt.output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
